...
def get_top_5_sellers(min_date, max_date, return_fig: bool | None = True):
    all_df = get_dataframe(min_date, max_date)

    top_seller_geo = (
        all_df.groupby("seller_id")
        .agg(
            {
                "geolocation_lat_seller": "first",
                "geolocation_lng_seller": "first",
                "payment_value": "sum",
            }
        )
    ...
    fig = px.bar(
        ...
        x="seller_id",
        y="payment_value",
        title="Total 5 Revenue Seller",
    )
    if not return_fig:
        return top_seller_geo
    return fig


def get_geomap_sellers(min_date, max_date):
    top_seller_geo = get_top_5_sellers(min_date, max_date, return_fig=False).head(1000)
    fig = go.Figure()

    fig.add_trace(
        go.Scattergeo(
            lon=top_seller_geo["geolocation_lng_seller"],
            lat=top_seller_geo["geolocation_lat_seller"],
            mode="markers",
            marker=dict(
                size=10, color="blue", opacity=0.8, line=dict(width=0.5, color="white")
            ),
        )
    )

    fig.update_layout(
        title="Map of Seller Distribution",
        geo=dict(scope="world", projection_type="orthographic"),
    )
